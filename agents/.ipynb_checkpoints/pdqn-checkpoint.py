...
class P_DQN(Base_Agent):
    ...
    def __init__(self, config, env):
        ...
        # ----  Initialization  ----
        self.critic = DuelingDQN(self.state_dim, self.action_dim, self.critic_hidden_layers,).to(self.device)
    ...
    def update(self, memory):
        ...
        # q_loss *= 0.1
        #q_loss = (F.mse_loss(q1, q_next) + F.mse_loss(q2, q_next))/2
        logger.info(f"critic loss: {q_loss}")
    ...
    def load_models(self, actor_path, actor_param_path):
        ...
        self.critic.load_state_dict(torch.load(actor_path))
        self.actor.load_state_dict(torch.load(actor_param_path))
